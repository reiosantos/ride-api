...
class Rides:
    ...
    def approve_request_for_ride(cls, ride_id, request_id) -> bool:
        if not ride_id or not request_id:
            return False
        ride = cls.find_one_ride(ride_id)
        request = cls.find_one_ride_request(request_id)
        if not ride or not request:
            return False

        request_object = cls.RequestModal(ride_id, contact)
        request_object.status = cls.RequestStatus.pending
        cls.requests.append(request_object)
        return True
